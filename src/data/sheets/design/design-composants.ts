...
export const designComposants = designSheet({
  ...
  readingTime: "20 min",
  description:
    "Anatomie des composants, navigation, formulaires, états d'interface et responsive design.",
  accent: "pattern",
  nodes: {
    anatomieComposant: designNode({
      ...
      practice: {
        ...
        etapes: [
          "Choisis un bouton réel d'une interface.",
          "Décris son anatomie : label, icône, taille, zone tactile, couleur, état de focus.",
          "Liste ses variants nécessaires : primaire, secondaire, danger, avec ou sans icône, pleine largeur ou contenu.",
          "Ajoute les états default, hover, focus, active, disabled et loading, avec le comportement attendu.",
        ],
        output: "Fiche de composant bouton avec anatomie, variants, états et règles de contenu.",
        critere:
          "La fiche est suffisante si un développeur peut implémenter le bouton sans demander ce qui se passe en loading, disabled ou texte long.",
        piege:
          "Créer un composant global trop tôt. Si un pattern n'a qu'un seul usage stable, le transformer en composant réutilisable peut rigidifier inutilement le produit.",
      },
      verification: [
        "Pourquoi un composant est-il un contrat plutôt qu'un simple morceau de maquette ?",
        "Ton bouton fonctionne en desktop avec un texte court, mais casse avec un texte de 38 caractères sur mobile. Quelle partie de la spécification manquait ?",
        "Quel trade-off existe entre composant très spécifique et composant très universel ?",
      ],
    }),
    navigation: designNode({
      ...
      system:
        'La navigation dépend de l\'architecture d\'information issue de la hiérarchie <span class="ref-fiche">→ D01</span> et du flux utilisateur <span class="ref-fiche">→ D01</span>. Elle influence les composants <span class="ref-fiche">→ D03</span>, le responsive <span class="ref-fiche">→ D03</span> et les choix visuels de repérage <span class="ref-fiche">→ D02</span> comme contraste, position et état actif.',
      choice:
        "Une barre supérieure convient aux produits avec peu de sections principales ; une sidebar supporte des outils denses et des hiérarchies persistantes ; une bottom navigation sert les actions mobiles fréquentes, souvent 3 à 5 destinations ; un breadcrumb clarifie une profondeur ; la recherche devient navigation quand l'espace informationnel est trop large pour être parcouru manuellement.",
      senior:
        "Un designer expérimenté ne choisit pas un pattern parce qu'il est tendance : il regarde fréquence d'usage, profondeur, changement de contexte et taille d'écran. Il redoute le menu hamburger utilisé pour cacher une architecture mal priorisée. Sur mobile, il vérifie aussi que les destinations critiques restent accessibles sans chasse au menu.",
      errors:
        "<p><strong>Pattern 1 — Le hamburger pansement :</strong> on cache la complexité dans un menu au lieu de prioriser les destinations. La cause est l'évitement : il est plus facile de cacher que de décider ce qui compte.</p><p><strong>Pattern 2 — Le labyrinthe profond :</strong> l'utilisateur descend dans plusieurs niveaux sans repère clair pour remonter. Ce pattern vient souvent d'une structure calquée sur l'organisation interne plutôt que sur les tâches utilisateur.</p><p><strong>Pattern 3 — L'état actif muet :</strong> la navigation existe mais ne signale pas où l'utilisateur se trouve. La cause est une vision décorative de la nav, oubliant son rôle d'orientation.</p>",
      invariants:
        "<p>Les patterns changent avec les devices, mais l'orientation reste un besoin cognitif stable. <strong>Ce qui change :</strong> les formats de navigation, gestes et conventions selon écran. <strong>Ce qui ne change pas :</strong> un utilisateur doit toujours pouvoir comprendre sa position, ses options et son chemin de retour.</p>",
      practice: {
        titre: "Choisir un pattern de navigation",
        etapes: [
          "Liste les destinations principales d'un produit réel ou imaginé.",
          "Classe-les par fréquence d'usage et criticité utilisateur.",
          "Choisis un pattern : top nav, sidebar, bottom nav, breadcrumb, recherche ou combinaison limitée.",
          "Justifie le choix avec profondeur, fréquence, taille d'écran et besoin de retour.",
        ],
        output:
          "Décision de navigation argumentée avec destinations prioritaires et pattern choisi.",
        critere:
          "La décision est solide si chaque destination visible a une raison liée à l'usage, pas seulement à l'organisation interne.",
        piege:
          "Traiter la navigation comme une liste exhaustive. La navigation principale est une sélection, pas un plan complet de l'entreprise.",
      },
      verification: [
        "Quelles trois questions une bonne navigation doit-elle résoudre en continu ?",
        "Une app mobile bancaire cache \"virement\" dans un hamburger alors que c'est l'action la plus fréquente. Quel pattern proposerais-tu et pourquoi ?",
        "Pourquoi la navigation dépend-elle davantage des tâches utilisateur que de la structure interne de l'entreprise ?",
      ],
    }),
    formulaires: designNode({
      ...
      senior:
        "Un designer expérimenté vérifie d'abord si chaque champ mérite d'exister. Il sait qu'un placeholder ne remplace jamais un label : dès que l'utilisateur saisit, l'aide disparaît. Il anticipe l'autofill, le clavier mobile, les formats locaux, les messages d'erreur récupérables et l'ordre logique de saisie.",
      errors:
        "<p><strong>Pattern 1 — Le champ par curiosité :</strong> l'équipe demande une information parce qu'elle pourrait servir plus tard. La cause est l'appétit de données, mais l'utilisateur paie immédiatement le coût de saisie.</p><p><strong>Pattern 2 — Le placeholder fantôme :</strong> le label est caché dans le placeholder, puis disparaît quand l'utilisateur écrit. Ce pattern vient d'une recherche de sobriété visuelle au détriment de la mémoire et de l'accessibilité.</p><p><strong>Pattern 3 — L'erreur punitive :</strong> l'utilisateur découvre plusieurs erreurs après soumission, avec des messages vagues. La cause est une validation pensée côté système plutôt que côté récupération utilisateur.</p>",
      invariants:
        "<p>Les champs et méthodes de saisie évoluent, mais demander une information reste un coût. <strong>Ce qui change :</strong> les contrôles, claviers, autofill et modes de validation. <strong>Ce qui ne change pas :</strong> chaque information demandée doit justifier son effort et chaque erreur doit indiquer comment reprendre.</p>",
      practice: {
        titre: "Réduire l'abandon d'un formulaire",
        etapes: [
          "Choisis un formulaire de 5 champs ou plus.",
          "Pour chaque champ, écris pourquoi l'information est nécessaire maintenant.",
          "Identifie les champs qui peuvent être supprimés, préremplis ou déplacés plus tard.",
          "Réécris deux messages d'erreur pour indiquer le problème, la cause probable et la correction attendue.",
        ],
        output:
          "Audit de formulaire avec justification des champs, suppressions possibles et deux messages d'erreur réécrits.",
        critere:
          "Un champ est légitime si tu peux expliquer sa valeur immédiate pour l'utilisateur ou le service rendu, pas seulement pour une analyse future.",
        piege:
          "Optimiser uniquement la longueur. Un formulaire plus court mais ambigu peut être moins performant qu'un formulaire légèrement plus long et mieux guidé.",
      },
      verification: [
        "Pourquoi un placeholder ne remplace-t-il pas un label ?",
        "Un formulaire d'inscription perd 65% des utilisateurs au champ téléphone. Quelles hypothèses tester avant de changer la couleur du bouton ?",
        "Quel trade-off existe entre validation inline et validation à la soumission ?",
      ],
    }),
    ...
    responsive: designNode({
      ...
      system:
        'Le responsive force à prioriser la hiérarchie <span class="ref-fiche">→ D01</span>, à préserver la lisibilité typographique <span class="ref-fiche">→ D02</span>, à adapter navigation et composants <span class="ref-fiche">→ D03</span>, puis à documenter ces règles dans le design system <span class="ref-fiche">→ D04</span>.',
      choice:
        "Le mobile-first oblige à commencer par l'essentiel puis enrichir ; le layout fluide adapte les dimensions entre breakpoints ; l'adaptatif prévoit des compositions différentes pour des seuils précis ; certaines interfaces denses nécessitent une version mobile repensée plutôt qu'une simple réorganisation. Les zones tactiles doivent rester au minimum proches de 44x44 px pour éviter les erreurs de toucher.",
      senior:
        "Un designer expérimenté ne teste pas seulement trois largeurs propres. Il vérifie les textes longs, le zoom système, le clavier ouvert, les zones tactiles, les orientations, les états d'erreur et les contenus réels. Il redoute les breakpoints choisis selon des devices populaires plutôt que selon le moment où le contenu casse.",
      errors:
        "<p><strong>Pattern 1 — Le desktop rétréci :</strong> l'équipe empile tout le desktop sur mobile sans repenser la priorité. La cause est la paresse de transformation : déplacer semble moins risqué que décider.</p><p><strong>Pattern 2 — Le breakpoint catalogue :</strong> les breakpoints sont choisis pour quelques modèles d'appareils, pas pour les ruptures réelles du layout. Le design casse dès qu'un écran intermédiaire ou un texte long apparaît.</p><p><strong>Pattern 3 — Le doigt oublié :</strong> les cibles tactiles restent dimensionnées pour une souris. La cause est un test effectué sur écran de design, sans interaction physique réelle.</p>",
      invariants:
        "<p>Les écrans changent de taille, densité et forme, mais le contenu a toujours des limites de lisibilité et l'interaction a toujours des contraintes physiques. <strong>Ce qui change :</strong> les devices, breakpoints et techniques CSS. <strong>Ce qui ne change pas :</strong> une interface doit rester lisible, actionnable et priorisée dans le contexte réel où elle est utilisée.</p>",
      practice: {
        titre: "Adapter un écran desktop au mobile",
        etapes: [
          "Choisis un écran desktop dense.",
          "Liste les éléments indispensables à la tâche principale sur mobile.",
          "Décide ce qui reste visible, ce qui devient secondaire et ce qui est supprimé ou déplacé.",
          "Vérifie les cibles tactiles, textes longs, clavier ouvert et ordre de lecture.",
        ],
        output:
          "Plan responsive mobile avec priorités visibles, éléments secondaires, règles d'interaction et cas de test.",
        critere:
          "Le plan est correct si la tâche principale peut être accomplie sur mobile sans zoom, sans cible minuscule et sans lecture désordonnée.",
        piege:
          "Penser le responsive comme un problème de grille uniquement. La vraie question est la priorité d'usage dans un contexte différent.",
      },
      verification: [
        "Pourquoi mobile-first aide-t-il à prioriser plutôt qu'à simplement réduire ?",
        "Un tableau desktop de 12 colonnes doit être consulté sur mobile par des techniciens terrain. Quelles alternatives peux-tu envisager ?",
        "Pourquoi les contraintes physiques du toucher restent-elles invariantes malgré l'évolution des frameworks ?",
      ],
    }),
  },
  maps: {
