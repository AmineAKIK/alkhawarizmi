...
export const designCode = designSheet({
  ...
  readingTime: "25 min",
  description:
    "Design system, préparation d'un fichier Figma, import, export, Community, design tokens, handoff et itération design-dev comme continuité entre intention et produit réel.",
  accent: "systeme",
  nodes: {
    ...
    ressourcesFigma: designNode({
      ...
      system:
        'L\'import fait entrer dans Figma des fichiers de travail ou des médias externes ; l\'export fait sortir une sélection utile au partage, au handoff ou au code <span class="ref-fiche">→ D04</span>. Dans Figma Design, une exportation peut porter sur des calques, frames, composants, groupes, sections, une portion du canvas ou le fichier entier. Community complète ce flux en permettant de rechercher notamment des UI kits, wireframes, modèles et extensions comme les plugins. Ces ressources peuvent accélérer les composants <span class="ref-fiche">→ D03</span>, mais elles doivent être évaluées et documentées comme toute ressource collaborative <span class="ref-fiche">→ Co14</span>.',
      choice:
        "Importe un fichier .fig ou .sketch depuis le navigateur de fichiers lorsque tu dois récupérer un document de travail ; ajoute directement au fichier de design les images, vidéos ou SVG nécessaires à la maquette, en gardant à l'esprit qu'un SVG importé devient un calque vectoriel éditable. À l'export, choisis selon l'usage : JPG pour une image web légère sans transparence, PNG pour préserver transparence et lisibilité, SVG pour une icône ou illustration vectorielle responsive, PDF pour partager une mise en page fixe. Dans Community, filtre la recherche selon le besoin : fichiers et modèles pour une base de travail, plugins pour une capacité précise. Duplique ou installe seulement après avoir lu la description et vérifié que la ressource apporte plus qu'elle ne complique.",
      senior:
        "Un designer expérimenté traite chaque import comme une matière à nettoyer, pas comme une vérité prête à livrer : il vérifie la hiérarchie des calques, les composants, les variantes, les noms et le poids des médias. Avant l'export, il demande au développement le format, la taille, l'échelle et le canal de livraison attendus au lieu de déposer plusieurs versions mystérieuses. Pour Community, il sépare inspiration, accélérateur temporaire et dépendance durable. Il vérifie l'origine, les conditions d'usage et le coût de maintenance d'une ressource ; il reste particulièrement prudent avec un plugin quand le fichier contient des informations sensibles.",
      errors:
        "<p><strong>Pattern 1 — L'import poubelle :</strong> une ressource externe est déposée dans le fichier puis utilisée telle quelle, avec des calques opaques et des composants incohérents. La cause est la confusion entre gagner du temps au départ et disposer d'un asset maintenable.</p><p><strong>Pattern 2 — L'export au hasard :</strong> l'équipe génère PNG, JPG et SVG sans définir le besoin, l'échelle ni la destination. Ce pattern vient d'un handoff implicite : le développement reçoit des fichiers mais pas une décision exploitable.</p><p><strong>Pattern 3 — Le buffet Community :</strong> kits et plugins s'accumulent parce qu'ils semblent utiles ou populaires, sans vérifier leur apport réel ni leur maintenance. La cause est l'attrait du raccourci visible alors que son coût futur reste diffus.</p>",
      invariants:
        "<p>Les formats et catalogues évoluent, mais une ressource externe doit toujours être choisie, inspectée et transmise consciemment. <strong>Ce qui change :</strong> les types de fichiers pris en charge, les formats d'exportation, les filtres Community et les extensions disponibles. <strong>Ce qui ne change pas :</strong> l'équipe doit connaître l'origine d'une ressource, vérifier sa qualité, choisir un format adapté à l'usage et éviter qu'un outil opportuniste devienne une dépendance incomprise.</p>",
      practice: {
        titre: "Constituer un mini-flux de ressources Figma",
        etapes: [
          "Importe dans un fichier Figma une image et un SVG utiles à un écran, puis inspecte leur comportement et la structure créée dans le panneau de calques.",
          "Sélectionne une icône ou une illustration, prépare deux exports justifiés parmi PNG, JPG, SVG et PDF, puis note pour chaque format sa destination et son compromis.",
          "Dans Community, recherche un UI kit, un modèle ou un wireframe adapté au même écran ; lis sa description, duplique-le dans un espace d'exploration et relève ce que tu conserverais ou retirerais.",
          "Recherche ensuite un plugin répondant à un besoin précis et rédige une décision courte : utilité, origine, coût de maintenance et précautions avant installation.",
        ],
        output:
          "Un tableau de ressources avec origine, type d'import, structure inspectée, exports retenus, destination et décision argumentée pour une ressource Community et un plugin.",
        critere:
          "Chaque import, export ou ressource Community doit avoir un usage explicite, un compromis connu et une place claire dans le fichier ou le handoff.",
        piege:
          "Mesurer la qualité du travail au nombre de ressources importées et de plugins installés. Une ressource utile réduit une difficulté précise sans rendre le fichier plus opaque.",
      },
      verification: [
        "Pourquoi un SVG importé et un PNG importé ne se manipulent-ils pas de la même manière dans Figma ?",
        "Quel format choisis-tu pour une icône responsive, une photographie web légère et une maquette fixe à partager, et pourquoi ?",
        "Avant de dupliquer un fichier Community ou d'installer un plugin, quelles informations vérifies-tu pour éviter une dépendance mal maîtrisée ?",
      ],
    }),
    ...
    iterationDesignDev: designNode({
      ...
      system:
        'L\'itération design-dev ferme la boucle entre expérience visée <span class="ref-fiche">→ D01</span>, composants conçus <span class="ref-fiche">→ D03</span>, design system <span class="ref-fiche">→ D04</span> et code produit <span class="ref-fiche">→ T04</span>. Elle alimente aussi la mesure et l\'apprentissage <span class="ref-fiche">→ C06</span> : le design n\'est validé que quand le produit réel produit le comportement attendu.',
      choice:
        "Le design ahead prépare un ou deux sprints d'avance pour réduire les blocages ; le design en parallèle convient aux petites équipes très synchronisées ; le pairing design-dev résout vite les interactions ambiguës ; la design QA vérifie l'implémentation réelle contre l'intention, sans tomber dans le pixel-perfect aveugle.",
      senior:
        "Un designer expérimenté sait que \"pixel-perfect\" est souvent une mauvaise question. Il vérifie plutôt si la hiérarchie, les états, l'accessibilité, les comportements et les intentions sont préservés dans le code. Il anticipe les compromis : parfois il faut ajuster le design pour une meilleure maintenabilité, parfois il faut défendre un détail parce qu'il porte la compréhension utilisateur.",
      errors:
        "<p><strong>Pattern 1 — La maquette intouchable :</strong> toute contrainte technique est vécue comme une dégradation du design. La cause est l'attachement au livrable plutôt qu'à l'intention utilisateur.</p><p><strong>Pattern 2 — Le dev comme exécutant :</strong> le développeur est consulté seulement après validation, donc ses connaissances de performance, responsive et accessibilité arrivent trop tard. Ce pattern vient d'une séparation hiérarchique des métiers.</p><p><strong>Pattern 3 — La QA cosmétique :</strong> la revue se limite à comparer les pixels, sans vérifier les états, le clavier, les données réelles ou les erreurs. La cause est que le visible immédiat attire plus l'attention que le comportement systémique.</p>",
      invariants:
        "<p>Les méthodes de production changent, mais une interface réelle est toujours le résultat d'un compromis entre intention, contrainte et usage observé. <strong>Ce qui change :</strong> les cycles de sprint, outils de QA, frameworks frontend et modes de collaboration. <strong>Ce qui ne change pas :</strong> design et code doivent itérer ensemble pour préserver l'expérience quand elle rencontre les contraintes du produit réel.</p>",
      practice: {
        titre: "Structurer une revue design-dev",
        etapes: [
          "Choisis une fonctionnalité en cours ou imagine une fonctionnalité prête à intégrer.",
          "Prépare une liste de vérification : hiérarchie, états, responsive, accessibilité, tokens, données longues, performance perçue.",
          "Décide quels points doivent être testés dans le navigateur plutôt que dans la maquette.",
          "Formule trois compromis acceptables et trois éléments à défendre absolument.",
        ],
        output:
          "Plan de revue design-dev avec checklist, tests navigateur, compromis et points non négociables.",
        critere:
          "La revue est utile si elle distingue clairement un écart cosmétique, un écart d'intention UX et une contrainte technique légitime.",
        piege:
          "Utiliser la revue design-dev comme tribunal de conformité. Le but est d'améliorer le produit réel, pas de prouver que la maquette avait toujours raison.",
      },
      verification: [
        "Pourquoi le pixel-perfect strict peut-il être une mauvaise métrique de qualité ?",
        "Un développeur explique qu'une animation prévue coûte trop cher en performance sur mobile. Comment raisonnes-tu le compromis ?",
        "Pourquoi l'expérience réelle ne peut-elle pas être validée uniquement dans Figma ?",
      ],
    }),
  },
  maps: {
