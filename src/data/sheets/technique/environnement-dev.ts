...
export const environnementDev: DevSheet = {
  ...
  nodes: {
    vscode: {
      ...
      niveau: "Fondation",
      sections: {
        why: `<p>Avant les éditeurs modernes, on codait dans des éditeurs sans contexte — le fichier, rien d'autre. Pas d'erreurs en temps réel, pas d'autocomplétion, pas d'intégration avec l'environnement. <strong>VS Code est né du constat que l'éditeur doit être un environnement, pas un bloc-notes amélioré.</strong></p>`,
        system: `<p>VS Code est le hub central du workflow. Il s'intègre avec <strong>Git</strong> (visualisation des diffs, staging inline), avec le <strong>terminal intégré</strong> (accès direct à WSL, bash, zsh sans changer de fenêtre), et avec le <strong>LSP</strong> (Language Server Protocol) qui alimente l'autocomplétion et les erreurs en temps réel.</p><p>Sur Windows, l'extension "Remote - WSL" fait tourner VS Code dans le contexte Linux, rendant la frontière entre les deux OS transparente.</p>`,
        choice: {
          kind: "structured",
          ...
          alternatives: [
            {
              name: "Cursor",
              description:
                "Fork de VS Code avec IA intégrée nativement. Recommandé quand tu maîtrises déjà VS Code.",
            },
            {
              name: "WebStorm / IntelliJ",
              description:
                "Plus puissant pour les gros projets Java/Kotlin. Payant, plus lourd. Sens dans un contexte entreprise.",
            },
            {
              name: "Neovim",
              description:
                "Vitesse maximale, entièrement au clavier. Courbe d'apprentissage abrupte. Pour plus tard.",
            },
          ],
        },
        senior: `<p>Un senior configure d'abord, code ensuite. Il installe ses extensions critiques (<strong>ESLint, Prettier, GitLens</strong>), active Settings Sync pour retrouver son environnement sur n'importe quelle machine, et configure le terminal intégré pour pointer sur le bon shell (WSL sur Windows, zsh sur macOS).</p><p>Il ne passe pas 3 heures à "personnaliser le thème" avant d'avoir écrit une ligne.</p>`,
        errors: `<p><strong>Pattern 1 — L'accumulation aveugle :</strong> accumuler des extensions sans comprendre ce qu'elles font. Résultat : éditeur lent, conflits silencieux, comportements imprévisibles.</p><p><strong>Pattern 2 — Ignorer le formatteur :</strong> ne pas configurer Prettier avec sauvegarde automatique. Le premier PR en équipe sera un enfer de diffs de formatage.</p><p><strong>Pattern 3 — Fuir le terminal :</strong> continuer à utiliser un terminal externe au lieu du terminal intégré. On perd la synchronisation de contexte.</p>`,
        invariants: `<p>L'éditeur est un outil de feedback rapide. Peu importe l'outil futur, le besoin reste : <strong>voir les erreurs sans compiler, naviguer vite dans la base de code, intégrer le contrôle de version visuellement.</strong></p><p>Ce qui change : l'interface. Ce qui ne change pas : la nécessité de réduire le temps entre "j'écris une erreur" et "je la vois".</p>`,
        practice: {
          commands: [
          ...
          verification:
            "code --version doit répondre, et les extensions essentielles doivent être installées dans le bon profil VS Code.",
    ...
    wsl: {
      ...
      sections: {
        why: `<p>Windows et Unix ont des philosophies d'OS radicalement différentes. La majorité des outils de dev (scripts, serveurs, Docker, pipelines CI) sont nés dans l'écosystème Unix et se comportent mal sous Windows — <strong>paths différents (\\ vs /), permissions différentes, scripts shell incompatibles.</strong></p><p>WSL (Windows Subsystem for Linux) fait tourner un vrai noyau Linux dans Windows, sans machine virtuelle dédiée. La frontière s'efface.</p>`,
        system: `<p>WSL est <strong>la fondation de tout l'environnement de dev sur Windows.</strong> Git, Node.js, Python — tout tourne à l'intérieur de WSL. VS Code se connecte à WSL via "Remote - WSL", rendant l'expérience transparente. C'est le pont entre l'OS de bureau et l'environnement de dev réel.</p>`,
        choice: {
          kind: "structured",
        ...
        practice: {
          ...
          verification:
            "wsl --status doit indiquer WSL2, et les projets doivent vivre dans le filesystem Linux plutôt que dans /mnt/c/.",
    ...
    ubuntu: {
      ...
      sections: {
          ...
          alternatives: [
            {
              ...
              description:
                "Recommandé. Support 5 ans, documentation abondante, compatibilité maximale.",
            },
            {
              name: "Debian",
              description:
                "Plus léger, ultra-stable. Paquets plus vieux. Bon choix serveur, moins pour dev moderne.",
            },
            {
              name: "Fedora (WSL)",
              description:
                "Utilise dnf à la place d'apt. Paquets plus récents. Si tu connais déjà l'écosystème Red Hat.",
            },
          ],
        },
        ...
        practice: {
          ...
          verification:
            "lsb_release -a doit afficher une version LTS attendue, puis apt update/upgrade doit s'exécuter sans erreur bloquante.",
    ...
    git: {
      ...
      sections: {
        ...
        practice: {
          ...
          verification:
            "git --version doit répondre, et git config --global --list doit contenir user.name, user.email et init.defaultBranch.",
    ...
    nodejs: {
      ...
      niveau: "Fondation",
      sections: {
        why: `<p>JavaScript était confiné au navigateur — impossible de l'utiliser côté serveur ou pour des scripts système. <strong>Node.js a extrait le moteur V8 de Chrome et l'a rendu exécutable en dehors du navigateur</strong>, créant un runtime JavaScript universel.</p><p>Résultat : le même langage côté client ET côté serveur, un écosystème npm de 2M+ de bibliothèques, et la base de tout projet frontend moderne (bundlers, transpileurs, frameworks).</p>`,
        system: `<p>Node.js est le runtime qui fait tourner ton code JS/TS. <strong>npm</strong> (inclus) est le gestionnaire de paquets qui donne accès à l'écosystème. Il est nécessaire pour tout projet frontend moderne — React, Vue, etc. — même si le code final tourne dans le navigateur.</p><p>Il s'installe via <strong>nvm</strong> (Node Version Manager), qui s'intercale entre l'OS et Node.js pour permettre de switcher de version selon le projet.</p>`,
        choice: {
          kind: "structured",
          ...
          alternatives: [
            {
              name: "nvm (recommandé)",
              description:
                "Gestionnaire de versions Node. Obligatoire pour gérer plusieurs projets avec des versions différentes.",
            },
            {
              name: "Bun",
              description:
                "Ultra-rapide (Zig), remplace Node.js + npm + bundler. Compatibilité npm excellente. À considérer pour de nouveaux projets.",
            },
            {
              name: "Deno",
              description:
                "Même créateur que Node, TypeScript natif, plus sécurisé. Écosystème plus petit. Intéressant mais pas le défaut.",
            },
          ],
        },
        senior: `<p>Il ne touche jamais à <code>apt install nodejs</code> — les dépôts Ubuntu ont souvent Node 12 alors qu'on est à Node 22. Tout passe par nvm.</p><p>Il utilise un fichier <strong><code>.nvmrc</code></strong> dans chaque projet avec la version requise. Quand il entre dans le projet, <code>nvm use</code> lit ce fichier et switch automatiquement. Zéro "ça marchait sur ma machine".</p><p>Il distingue clairement <code>dependencies</code> et <code>devDependencies</code> dans package.json.</p>`,
        errors: `<p><strong>Pattern 1 — L'install directe :</strong> <code>sudo apt install nodejs</code> → version obsolète + conflit avec nvm dans le PATH. Désinstaller et repartir sur nvm.</p><p><strong>Pattern 2 — node_modules dans Git :</strong> oublier <code>node_modules/</code> dans <code>.gitignore</code> → dépôt de 200MB, CI cassé, dépendances sur-committées.</p><p><strong>Pattern 3 — La version instable :</strong> <code>nvm install node</code> sans préciser de version → récupère la latest, pas forcément la LTS.</p>`,
        invariants: `<p>Le runtime est distinct du langage. JavaScript peut tourner dans un navigateur, dans Node.js, dans Deno, dans Bun. <strong>Ce qui change : le runtime, son gestionnaire de paquets et ses performances. Ce qui ne change pas : le besoin d'un moteur qui exécute JavaScript hors du navigateur avec des dépendances reproductibles.</strong></p>`,
        practice: {
          commands: [
          ...
          verification:
            "node --version et npm --version doivent répondre depuis le même shell, et nvm current doit correspondre à la version attendue.",
      ...
      sections: {
        ...
        choice: {
          ...
          alternatives: [
            {
              name: "venv (standard)",
              description: "Intégré à Python 3, léger, suffisant pour 95% des cas.",
            },
            {
              name: "pyenv",
              description:
                "Comme nvm pour Python. Gère plusieurs versions de Python. Recommandé si tu jongle entre des projets Python 3.9 / 3.11 / 3.12.",
            },
            {
              name: "uv",
              description:
                "Nouveau (Rust), ultra-rapide, remplace pip + venv + pyenv en un outil. L'avenir probable. À surveiller.",
            },
            {
              name: "conda",
              description:
                "Standard data science. Gère aussi les environnements. Lourd pour un projet backend classique.",
            },
          ],
        },
        senior: `<p>Il crée <strong>toujours un venv par projet</strong>. Il ne pip-installe jamais globalement — c'est une pollution qui crée des conflits invisibles entre projets.</p><p>Il vérifie toujours avec <code>python3 --version</code> — selon le système, <code>python</code> peut pointer sur Python 2. Il pin ses dépendances dans <code>requirements.txt</code> ou <code>pyproject.toml</code> pour la reproductibilité.</p>`,
        errors: `<p><strong>Pattern 1 — L'install globale :</strong> <code>pip install django</code> sans activer un venv → le package s'installe globalement, pollue l'environnement système, crée des conflits entre projets.</p><p><strong>Pattern 2 — Le venv oublié :</strong> travailler dans un projet sans activer le venv → les imports trouvent les mauvaises versions ou ne trouvent rien.</p><p><strong>Pattern 3 — La confusion python/python3 :</strong> sur certains systèmes, <code>python</code> pointe sur Python 2. Toujours utiliser <code>python3</code> explicitement.</p>`,
        invariants: `<p>La gestion d'environnements isolés par projet est un besoin universel en dev. <strong>Ce qui change : l'outil d'isolation, venv, conda, uv, Docker ou autre. Ce qui ne change pas : les dépendances d'un projet ne doivent pas affecter les autres projets de la machine.</strong></p>`,
        practice: {
          commands: [
          ...
          verification:
            "python3 --version doit répondre, et which python après activation doit pointer vers le dossier .venv du projet.",
    ...
    github: {
      ...
      niveau: "Fondation",
      sections: {
        why: `<p>Git est local — il versionne ton code sur ta machine. Sans remote, Git ne sauvegarde pas dans le cloud et ne permet pas la collaboration. <strong>GitHub est le remote : il héberge les dépôts, facilite la collaboration via les Pull Requests, et s'est imposé comme la plateforme sociale du code.</strong></p>`,
        system: `<p>GitHub est le bout de la chaîne : Local (Git) → Remote (GitHub). Il s'intègre aussi avec les pipelines CI/CD via <strong>GitHub Actions</strong>, les revues de code (Pull Requests, code review), et la gestion de projet (Issues, Projects, Milestones).</p><p>Il se connecte à Git via SSH — une clé qu'on génère une fois et qui authentifie tous les push/pull ensuite.</p>`,
        choice: {
          kind: "structured",
          ...
          alternatives: [
            {
              name: "GitHub",
              description:
                "Standard. Plus grand écosystème, GitHub Actions, communauté open source. Défaut absolu.",
            },
            {
              name: "GitLab",
              description:
                "DevOps plus complet intégré nativement. Auto-hébergeable. Bon si l'équipe a des besoins CI/CD avancés.",
            },
            {
              name: "Bitbucket",
              description:
                "Atlassian. Pertinent si l'équipe utilise Jira. Pas de raison de choisir sinon.",
            },
          ],
        },
        senior: `<p>Il configure <strong>SSH dès le départ</strong> — pas de HTTP avec mot de passe à chaque push. Une clé <code>ed25519</code> générée une fois, ajoutée sur GitHub, et tous les dépôts sont accessibles sans friction.</p><p>Il sait que <strong>GitHub n'est pas Git</strong>. C'est un service au-dessus de Git, avec ses propres concepts (fork, PR, Actions). Les confondre crée des incompréhensions fondamentales.</p>`,
        errors: `<p><strong>Pattern 1 — Les secrets exposés :</strong> pousser un fichier <code>.env</code> avec des tokens ou mots de passe dans un dépôt public. Exposé en secondes, indexé par des bots. Le <code>.gitignore</code> est non-négociable.</p><p><strong>Pattern 2 — Le commit direct sur main :</strong> en équipe, travailler directement sur <code>main</code> sans branches ni Pull Requests → conflits, historique pollué, pas de revue de code.</p><p><strong>Pattern 3 — La confusion push/sauvegardé :</strong> croire que push = code sauvegardé. Si tu n'as pas commité, push ne fait rien.</p>`,
        invariants: `<p>La distinction local/remote est fondamentale dans tout système distribué. <strong>Ce qui change : l'hébergeur, ses interfaces et ses automatisations. Ce qui ne change pas : le besoin d'un dépôt central partagé pour coordonner, sauvegarder et relire le travail.</strong></p>`,
        practice: {
          commands: [
            ...
            {
              type: "comment",
              value: "Copier la clé publique et l'ajouter dans GitHub > Settings > SSH Keys",
            },
          ...
          verification:
            "ssh -T git@github.com doit confirmer l'authentification, puis git remote -v doit afficher une URL SSH cohérente.",
    ...
    apt: {
      ...
      niveau: "Fondation",
      sections: {
        why: `<p>Linux ne vient pas avec un store pour les outils de dev. <strong>apt (Advanced Package Tool) est le gestionnaire de paquets de Debian/Ubuntu</strong> — il installe, met à jour, supprime des logiciels depuis des dépôts centralisés, avec résolution automatique des dépendances. Sans apt, chaque installation est manuelle et sans garde-fou.</p>`,
        system: `<p>apt est l'infrastructure d'installation sur Linux. <strong>Comme Homebrew sur macOS.</strong> Avant d'installer quoi que ce soit, <code>sudo apt update</code> synchronise les listes de paquets disponibles. Il gère les dépendances automatiquement — si git a besoin de libcurl, apt l'installe.</p>`,
        choice: {
          kind: "structured",
          ...
          alternatives: [
            {
              ...
              description: "Standard ici. Large écosystème, documentation abondante.",
            },
            {
              name: "dnf / yum (Fedora/RHEL)",
              description: "Équivalent sur les distributions Red Hat.",
            },
            {
              name: "pacman (Arch)",
              description:
                "Paquets plus récents, AUR communautaire. Distributions rolling-release.",
            },
          ],
        },
        ...
        practice: {
          ...
          verification:
            "apt --version doit répondre, et sudo apt update doit synchroniser les dépôts sans erreur critique.",
      ...
      sections: {
          ...
          alternatives: [
            {
              name: "Homebrew",
              description:
                "Standard absolu. Communauté énorme, formules à jour, intégration propre.",
            },
            {
              name: "MacPorts",
              description:
                "Plus ancien, moins populaire, installe dans /opt/local. Moins de formules. Pas de raison de choisir.",
            },
            {
              name: "Nix",
              description:
                "Gestion fonctionnelle et reproductible des paquets. Puissant, complexe. Pour des setups avancés.",
            },
          ],
        },
        ...
        practice: {
          commands: [
            ...
            {
              type: "comment",
              value: `Après installation, suivre les instructions "Next steps" dans le terminal`,
            },
          ...
          verification:
            "brew doctor doit ne signaler aucun problème bloquant, et brew --version doit répondre depuis un nouveau terminal.",
    ...
    clt: {
      ...
      sections: {
        ...
        choice: {
          ...
          alternatives: [
            {
              name: "CLT seuls (recommandé)",
              description:
                "Légers (~1GB), suffisants pour 99% du dev web/backend/mobile JS. Téléchargement rapide.",
            },
            {
              name: "Xcode complet",
              description:
                "10GB+. Nécessaire UNIQUEMENT pour dev iOS/macOS natif (Swift, Objective-C). Pas pour le web.",
            },
          ],
        },
        ...
        practice: {
          commands: [
            ...
            {
              type: "comment",
              value: "Une fenêtre graphique s'ouvre pour confirmer l'installation",
            },
          ...
          verification:
            "xcode-select -p doit retourner un chemin existant vers les Command Line Tools.",
    ...
    shell: {
      ...
      sections: {
        ...
        choice: {
          ...
          alternatives: [
            {
              name: "zsh (recommandé macOS)",
              description:
                "Défaut sur macOS depuis Catalina. Plus de features que bash, oh-my-zsh, plugins. Standard moderne.",
            },
            {
              name: "bash",
              description:
                "Universel, présent sur tout Unix. Plus compatible, moins de features. Défaut sur Linux.",
            },
            {
              name: "fish",
              description:
                "Syntax différente (non-POSIX), autocomplétion très avancée. Incompatible avec certains scripts bash. Pour plus tard.",
            },
            {
              name: "nushell",
              description:
                "Shell structuré ultra-moderne. Rupture complète avec le paradigme texte. Expérimental en prod.",
            },
          ],
        },
        ...
        practice: {
          commands: [
            ...
            {
              type: "cmd",
              value: `sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"`,
            },
          ],
          verification:
            "echo $SHELL doit afficher le shell attendu, et source ~/.zshrc ne doit pas produire d'erreur.",
