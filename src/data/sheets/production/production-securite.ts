...
export const productionSecurite = prodSheet({
  ...
  subtitle: "Réduire la surface d'attaque, protéger les accès et traiter les dépendances comme du code critique.",
  badge: "Fiche P02 — Vision Systémique",
  meta: ["5 nœuds · universel", "8 sections / nœud"],
  readingTime: "40 min",
  description: "Modèle de sécurité, secrets, auth, OWASP et dépendances pour penser la protection comme un système.",
  accent: "securite",
  nodes: {
    modeleSecurite: prodNode({
      ...
      system: "Le modèle de sécurité donne du sens à la gestion des secrets, à l'authentification, à l'autorisation, aux vulnérabilités OWASP et aux dépendances.",
      choice: "Les trois réflexes sont defense in depth, moindre privilège et threat modeling. On réduit d'abord la surface d'attaque, puis on protège ce qui reste exposé.",
      senior: "Il voit chaque endpoint public, permission, secret, webhook et dépendance comme une surface d'attaque. Un endpoint inexistant ne peut pas être attaqué.",
      ...
      practice: {
        ...
        critere: "Une app web standard expose toujours plusieurs surfaces; si tu n'en vois qu'une, l'audit est trop superficiel.",
        piege: "Penser que la sécurité appartient à quelqu'un d'autre. Chaque développeur sécurise le code qu'il écrit.",
      },
      verification: [
    ...
    secretsConfig: prodNode({
      ...
      system: "Les secrets sont la première ligne de défense et s'appuient sur les variables d'environnement, puis sur des gestionnaires dédiés quand l'équipe grandit.",
      choice: "Variables d'environnement au minimum, Doppler, Infisical ou 1Password Secrets pour l'équipe, Vault pour les contraintes fortes de conformité.",
      senior: "Il impose rotation et moindre privilège. Si un secret apparaît dans Git, son ordre d'action est non négociable: révoquer la clé, déployer une nouvelle clé, vérifier les accès, puis seulement nettoyer l'historique. Nettoyer Git avant révocation donne une fausse impression de sécurité.",
      ...
      practice: {
        ...
        commands: [
          ...
          { type: "cmd", value: "npm install --save-dev secretlint @secretlint/secretlint-rule-preset-recommend" },
          { type: "cmd", value: 'npx secretlint "**/*"' },
          { type: "comment", value: "Scanner l'historique Git" },
          { type: "cmd", value: 'docker run --rm -v "$PWD:/repo" trufflesecurity/trufflehog:latest git file:///repo --only-verified' },
          { type: "comment", value: "Secret commité: révoquer d'abord, remplacer ensuite, nettoyer l'historique après" },
        ],
    ...
    authnAuthz: prodNode({
      ...
      system: "L'auth protège les endpoints, les données et les opérations. Elle doit être appliquée côté serveur à chaque requête sensible.",
      choice: "JWT pour APIs stateless, sessions serveur pour apps traditionnelles, OAuth/OIDC ou providers comme Clerk, Auth0 ou Supabase Auth pour déléguer le risque.",
      senior: "Il ne fait jamais confiance au client. Cacher un bouton n'empêche pas l'appel direct de l'API. Sur un JWT, il vérifie la signature, l'expiration, l'audience, l'issuer et l'algorithme attendu; il sait aussi qu'un JWT compromis reste valide jusqu'à expiration sans liste de révocation.",
      ...
      practice: {
        ...
        commands: [
          ...
          { type: "snippet", value: 'const payload = jwt.verify(token, process.env.JWT_SECRET, { algorithms: ["HS256"], audience: "api", issuer: "auth" })' },
          { type: "comment", value: "Autorisation dans le handler" },
          { type: "snippet", value: "if (resource.ownerId !== req.user.id) return res.status(403).json({ error: 'Forbidden' })" },
    ...
    owaspTop10: prodNode({
      ...
      system: "Les catégories OWASP relient les décisions de code aux risques de production: injection, broken access control, crypto failures, auth failures et logging failures.",
      choice: "Pour débuter: A01 Broken Access Control, A02 Crypto Failures, A03 Injection, A07 Auth Failures et A09 Logging/Monitoring Failures.",
      senior: "Il traite Broken Access Control en priorité absolue parce qu'il est A01 dans l'OWASP Top 10 2021: le bug n'est souvent pas spectaculaire, juste un contrôle de propriété oublié. Injection reste critique, mais le premier réflexe de garde est de vérifier qui peut accéder à quelle ressource.",
      ...
      practice: {
        ...
        commands: [
          ...
          { type: "comment", value: "Éviter npm audit fix --force sans revue: il peut introduire des breaking changes" },
    ...
    dependances: prodNode({
      ...
      system: "La sécurité des dépendances appartient au cycle de développement et à la CI/CD. Dependabot, audits et lock files réduisent le risque de supply chain.",
      choice: "npm audit ou pip-audit pour détecter, Dependabot ou Renovate pour ouvrir les PRs, Snyk pour un suivi plus complet.",
      senior: "Il surveille autant les dépendances transitives que directes. Le lock file est une mesure de sécurité: il empêche les changements silencieux d'arbre de dépendances.",
